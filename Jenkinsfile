  ...
  node(POD_LABEL) {
    ...
    stage('Build Docker Image') {
      container('docker') {
        ...
        docker_image = docker.build("knowledgeintegration/${module_name}")
      }
    }
        ...
        dir('docker') {
          if ( checkout_details?.GIT_BRANCH == 'origin/main' ) {
            ...
            if ( !is_snapshot ) {
              ...
              env.MOD_RS_IMAGE="knowledgeintegration/${module_name}:${app_versionapp_version}"
              env.SERVICE_ID="${module_name}-${app_version}"
              ...
              // env.MOD_RS_IMAGE="knowledgeintegration/${module_name}:${app_version}.${BUILD_NUMBER}"
              env.MOD_RS_IMAGE="knowledgeintegration/${module_name}:${app_version}"
              env.SERVICE_ID="${module_name}-${app_version}.${BUILD_NUMBER}"
              env.MOD_RS_DEPLOY_AS=env.SERVICE_ID.replaceAll('\\.','-').toLowerCase();
            }
          }
          else {
    ...
    stage('Publish module descriptor') {
      if ( checkout_details?.GIT_BRANCH == 'origin/main' ) {
        ...
        // Not currently publishing this module descriptor to the great okapi in the sky
